...
class ConfigService {
  ...
  List<int> get resolution =>
      List<int>.from(getValue('display.resolution', [600, 1024]));
  ...
  int get metadataAutoHideDelay =>
      getValue('display.metadata_auto_hide_delay', 3);

  // Filter settings
  bool get filtersEnabled => getValue('filters.enabled', false);
  List<String> get filterSets =>
      List<String>.from(getValue('filters.sets', <String>[]));
  List<String> get filterColors =>
      List<String>.from(getValue('filters.colors', <String>[]));
  List<String> get filterCardTypes =>
      List<String>.from(getValue('filters.card_types', <String>[]));
  List<String> get filterCreatureTypes =>
      List<String>.from(getValue('filters.creature_types', <String>[]));
  List<String> get filterRarity =>
      List<String>.from(getValue('filters.rarity', <String>[]));
  ...
  Map<String, dynamic> _mergeConfigs(
      Map<String, dynamic> defaults, Map<String, dynamic> saved) {
    final merged = Map<String, dynamic>.from(defaults);

    saved.forEach((key, value) {
      if (merged[key] is Map<String, dynamic> &&
  ...
  T _getNestedValue<T>(
      Map<String, dynamic> config, String path, T defaultValue) {
  ...
  void _setNestedValue(
      Map<String, dynamic> config, String path, dynamic value) {
    final parts = path.split('.');
